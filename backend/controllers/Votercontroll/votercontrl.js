...
async function replacePasskey(req, res) {
  try {
    const votersToUpdate = await Voter.findAll({ where: { id: { [Sequelize.Op.lt]: 1000 } } });
...
const matchingPasskey = await Voter.findOne({where:{voterPasskey:passkey}})
if(!matchingPasskey){ 
return res.status(200).json({message:"No matching pass key"})
}
else if(matchingPasskey){
return res.status(200).setHeader("Authorization",token).json({message:"Passkey found",token})
}
...
module.exports = {postVoterandsentvoterpasskey,validatePasskey,countVoters,fetchAllemails,propagateMail}