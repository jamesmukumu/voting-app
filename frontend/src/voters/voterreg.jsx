...
function Registervoter() {
  ...
  const [scsmsg, setSucessmsg] = useState("");
  const [loading, setLoading] = useState(false);
  async function postvoter(e) {
    setLoading(true);
    e.preventDefault();
    try {
      ...
      if (response.data.message === "voter registration pass") {
        setLoading(false);
        setSucessmsg("Registration has occured successfully");
      } else if (response.data.error === "field must be unique") {
        setLoading(false);
        setSucessmsg("Fiel must be unique");
      }
    } catch (error) {
      setLoading(false);
      setSucessmsg("internal server error");
    }
  }

  return (
    <div className="form-iebc">
      <div className="intro">
        <img
