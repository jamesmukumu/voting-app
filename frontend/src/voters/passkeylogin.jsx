  ...
  async function postPasskey(e) {
    ...
    try {
      ...
      if (response.data.message === "No matching pass key") {
        setLoading(false);
        setErrmsg("Passkey invalid");
      } else if (response.data.message === "Passkey found") {
        setTimeout(() => {
          navigate("/cast/vote");
        }, 3000);
        setLoading(true);
        Cookie.set("access cookie", response.data.token);
        setErrmsg("Valid pass key");
      }
    } catch (error) {
    ...
    <div className="form-iebc">
      ...
      {loading ? (
        ...
        <form onSubmit={postPasskey}>
          ...
          <button>Enter Voting portal</button>
